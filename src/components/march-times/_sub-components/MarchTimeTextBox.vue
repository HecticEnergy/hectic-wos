<template>
  <v-text-field
    ...
    @update:model-value.capture="(e) => { console.log('modelUpdate', e)}"
    @blur="updateModel"
...
const updateModel = () => {
  ...
  const marchTimeValue = marchTime.value.padStart(4, "0");
  if (marchTime.value.length > 2) {
    if (marchTime.value.includes(":")) {
  ...
  model.value = { ...model.value, ...timeFromSec };
